    ...
    switch (message.type) {
      ...
      case "move":
        ...
        if (message.gameState) {
          ...
          // Call board's game state update handler
          const handleServerGameStateUpdate = getState(
            "handleServerGameStateUpdate",
            null
          );
          if (handleServerGameStateUpdate) {
            handleServerGameStateUpdate();
          }
        }
        break;
  ...
  return {
    render: () => {
      return {
        div: {
          className:
            "min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4",
          children: () => {
            ...
            return [
              {
                div: {
                  className:
                    "bg-white rounded-xl shadow-2xl p-8 w-full max-w-md",
                  children: [
                    // Title
                    {
                      h1: {
                        className:
                          "text-3xl font-bold text-gray-800 text-center mb-8",
                      ...
                      ? [
                          {
                            div: {
                              className:
                                "mb-6 p-4 bg-amber-50 rounded-lg border border-amber-200",
                              children: [
                                {
                                  h3: {
                                    className:
                                ...
                                },
                                {
                                  p: {
                                    className: "text-sm text-amber-700 mb-1",
                                    textContent: `Your Color: ${
                                      playerColor || "Not assigned"
                                    }`,
                                  },
                                },
                                {
                    ...
                    ...(!roomId && connectionStatus === "connected"
                      ? [
                          // Create Room
                          {
                            button: {
                              className:
                                "w-full bg-blue-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-blue-700 transition-colors mb-4",
                              ...
                              children: [
                                {
                                  input: {
                                    ...
                                    value: joinRoomCode,
                                    oninput: (e) => handleRoomCodeChange(e),
                                    maxlength: 6,
                                  },
                                },
                                {
                                  button: {
                                    className:
                                      "w-full bg-green-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-green-700 transition-colors",
